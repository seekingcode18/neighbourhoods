  ...
  pool.query(
    ...
    (error, results) => {
      ...
      res
        .status(201)
        .send(
          `House added with id of ${id}! Please visit /api/v1/houses to check!`
        );
    }
  );
};
...
const updateHouse = (req, res) => {
  ...
  pool.query(
    "UPDATE houses SET address_id = $2, number_of_occupants = $3 WHERE id = $1",
    [id, address_id, number_of_occupants],
    (error, results) => {
      if (error) {
        throw error;
...
const deleteHouse = (req, res) => {
  ...
  pool.query("DELETE FROM houses WHERE id=$1", [id], (error, results) => {
    if (error) {
      throw error;
    }
    res.status(200).send(`House deleted with id of ${id}`);
  });
};
  ...
  pool.query(
    ...
    (error, results) => {
      ...
      res
        .status(201)
        .send(
          `Person added with id of ${id}! Please visit /api/v1/people to check!`
        );
    }
  );
};
...
const updatePerson = (req, res) => {
  ...
  pool.query(
    "UPDATE people SET name = $2, age = $3 WHERE id = $1",
    [id, name, age],
    (error, results) => {
      if (error) {
        throw error;
...
const deletePerson = (req, res) => {
  ...
  pool.query("DELETE FROM people WHERE id=$1", [id], (error, results) => {
    ...
    res.status(200).send(`Person deleted with id of ${id}`);
  });
};
  ...
  pool.query(
    ...
    (error, results) => {
      ...
      res
        .status(201)
        .send(
          `Address modified with id of ${id}! Please visit /api/v1/addresses to check!`
        );
    }
  );
};
...
const updateAddress = (req, res) => {
  ...
  pool.query(
    "UPDATE addresses SET flat_number = $2, street_number = $3, building_name = $4, street_name = $5, postcode = $6 WHERE id = $1",
    [id, flat_number, street_number, building_name, street_name, postcode],
    (error, results) => {
      if (error) {
        throw error;
...
const deleteAddress = (req, res) => {
  ...
  pool.query("DELETE FROM addresses WHERE id=$1", [id], (error, results) => {
    ...
    res.status(200).send(`Address deleted with id of ${id}`);
  });
};
